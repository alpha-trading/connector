...
class Function:
    ...
    @staticmethod
    def get_ts_rank(
        value: Series, day: int, method: KindOfAverage = KindOfAverage.average
    ) -> Series:
        """
    ...
    def get_ts_regression(value: Series, day: int) -> Series:
        ts_regression = value.rolling(window=day, min_periods=day).apply(
            lambda x: linear_regression(
                [[x_val] for x_val in np.arange(1, day + 1)],
                [
    ...
    @staticmethod
    def get_ts_correlation(
        lhs_value: DataFrame, rhs_value: DataFrame, window_size: int
    ):
        merged = merge(
            left=lhs_value,
            right=rhs_value,
